...
export async function createThread({
    ...
    try {
        ...
        throw new Error(`Failed to create thread or update user as 
        ${error.message}`);
    }
}
...
export async function fetchThreads(pageNumber = 1, pageSize = 20){
    try {
        ...
        throw new Error(`Failed to fetch threads for the user as 
        ${error.message}`);
    }
}
...
export async function fetchThreadById(id : string){
    try {
        ...
        .populate({
            path : 'children',
            // this populate selects author info of comments
            populate : [{
                path : 'author',
                model : User,
                select : "_id id name parentId image"
            },
            // this populates the comment itself
            {
                path : 'children',
                model : Thread,
                populate : {
...
export async function addLikeToThread(id: String, userId : String) {
    try {
        connectToDB();
        await Thread.findByIdAndUpdate(id, {$push: {likes: userId}},
        {new : true });
    } catch (error: any) {
...
export async function addCommentToThread(threadId : string, commentText : string,
  ...
  try {
    ...
    throw new Error(`Failed to add comment to the post ${error.message}`);
  }
}